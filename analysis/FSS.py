...
This is from the pyhton package fssa, some changes to account for diffrent 
numpy and scipy versions and updated functionality. See: 
https://pyfssa.readthedocs.io/en/stable/reference/fssa.html 
...
def autoscale(l, rho, a, da, rho_c0, nu0, zeta0, x_bounds=None, **kwargs):
    ...
    """
    def goal_function(x):
            # Prevent physical impossibilities/unsorted arrays by returning infinity
            # if nu becomes non-positive.
            if x[1] <= 0:
                return np.inf
    ...
    # Define bounds: rho_c must be within data limits, nu must be positive (> 0)
    # None means unbounded
    rho = np.asanyarray(rho)
    bounds = [
        (rho.min(), rho.max()),  # rho_c bounds
        (1e-5, None),            # nu bounds (must be strictly positive)
        (None, None)             # zeta bounds
    ]

    # Modern SciPy Nelder-Mead uses 'xatol' and 'fatol' instead of 'xtol' and 'ftol'
...
def finite_size_scaling(l_list, rho, order_parameters, order_parameters_std, lower, upper, rho_c, nu, beta): 
    # Focus on critical region
    rho = rho[lower:upper]
    order_parameters = order_parameters[:, lower:upper]
    order_parameters_std = order_parameters_std[:, lower:upper]

    # Ensure rho is sorted from smallest to largest
    sort_indices = np.argsort(rho)
    rho = rho[sort_indices]
    order_parameters = order_parameters[:, sort_indices]
    order_parameters_std = order_parameters_std[:, sort_indices]

    if np.all(order_parameters_std == 0): 
